#pragma once
#include "GBAnisotropy1MisAng5.h"

registerMooseObject("yinglongApp", GBAnisotropy1MisAng5);

InputParameters
GBAnisotropy1MisAng5::validParams()
{
...
GBAnisotropy1MisAng5::GBAnisotropy1MisAng5(const InputParameters & parameters)
...
Real
GBAnisotropy1MisAng5::calculatedGBEnergy(const misoriAngle_isTwining & misori_gbType)
{
...
Real
GBAnisotropy1MisAng5::calculatedGBMobility(const misoriAngle_isTwining & misori_gbType)
{
  const Real B = 5;
  const Real n = 4;
  Real gbMob = _matrix_mob;
  const Real threshold_HAB = 15.0;
