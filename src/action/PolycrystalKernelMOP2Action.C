...
#include "PolycrystalKernelMOP2Action.h"
#include "Factory.h"
#include "Conversion.h"
#include "FEProblem.h"

registerMooseAction("yinglongApp", PolycrystalKernelMOP2Action, "add_kernel");

InputParameters
PolycrystalKernelMOP2Action::validParams()
{
...
PolycrystalKernelMOP2Action::PolycrystalKernelMOP2Action(const InputParameters & params)
  : Action(params),
...
void
PolycrystalKernelMOP2Action::act()
{
  std::cout << "the done of PolycrystalKernelMOP2Action" << std::endl;
  std::cout << "_op_num " << _op_num << std::endl;
  ...
  for (unsigned int op = 0; op < _op_num; ++op)
  {
    ...
    //
    // Set up ACGrGrPolyMOP2 kernels
    //

    {
      InputParameters params = _factory.getValidParams("ACGrGrPolyMOP2");
      params.set<NonlinearVariableName>("variable") = var_name;
      params.set<std::vector<VariableName>>("v") = v;
      params.set<unsigned int>("cur_op") = op; // for gamma_ij[cur_op][i]
      params.applyParameters(parameters());

      std::string kernel_name = "ACBulk_" + var_name;
      _problem->addKernel("ACGrGrPolyMOP2", kernel_name, params);
    }
